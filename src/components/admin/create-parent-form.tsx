...
export function CreateParentForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch("/api/admin/invite-user", {
      ...
      body: JSON.stringify({ fullName, email, role: "parent" }),
    });
    ...
    if (!response.ok) {
      setStatusType("error");
      setStatus(data.error ?? "Failed to invite parent.");
      return;
    }
    ...
    if (data.emailError) {
      ...
      setStatus("Account created and credentials emailed. Link children from the parents table.");
    }
  ...
  const handleResend = async () => {
    ...
    const response = await fetch("/api/admin/invite-user", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...pendingResend, role: "parent", resend: true }),
    });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base text-navy">Invite parent</CardTitle>
      </CardHeader>
      <CardContent>
