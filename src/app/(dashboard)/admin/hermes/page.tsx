...
export default async function HermesAdminPage() {
  ...
  const [contacts, cases, approvals, messages] = await Promise.all([
    supabase
  ...
  return (
    <HermesAssistantDashboard
      ...
      loadError={contacts.error || cases.error || approvals.error || messages.error ? "Some assistant information could not be loaded." : null}
    />
  );
}
