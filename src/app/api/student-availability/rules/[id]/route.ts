...
export async function PATCH(
  ...
  if (profile.role !== "student") {
    return NextResponse.json({ error: "Only students can update student availability." }, { status: 403 });
  }
...
export async function DELETE(
  ...
  if (!profile) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (profile.role !== "student") {
    return NextResponse.json({ error: "Only students can delete student availability." }, { status: 403 });
  }
