...
7. If the sender cannot confirm the exact class or whether the request concerns an individual enrollment or the whole occurrence after bounded clarification, stop the mutation and call `report_class_ambiguity={candidateOccurrenceIds,ambiguityKind,clientRequestId}` once. Use only 1–5 occurrence IDs returned by `find_my_classes` and `ambiguityKind` `class` or `scope`; never include message text, names, phone numbers, or a free-form reason. A later exact `confirm_class_selection` resolves that structured attention item.
8. Every class mutation requires a stable `clientRequestId`. Create it once for the intended action and reuse that exact value after a timeout or uncertain response. Never substitute the signed transport request ID.
...
| Escalate unresolved class/scope | `report_class_ambiguity` | 1–5 returned occurrence IDs, `class` or `scope` only |
