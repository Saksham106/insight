...
For five-minute retry and quiet-period maintenance, use Hermes's built-in scheduler. Copy `scripts/kitty-class-maintenance.py` into the Academy profile's `$HERMES_HOME/scripts/` directory, set `INSIGHT_KITTY_MAINTENANCE_URL=https://<insight-host>/api/cron/kitty-classes` in that profile, and reuse its existing `HERMES_TOOL_SHARED_SECRET`. Create the deterministic no-agent job with `hermes -p academy cron create "every 5m" --no-agent --script kitty-class-maintenance.py --deliver local --name "Kitty class maintenance"`. The gateway ticks every 60 seconds; verify with `hermes -p academy cron run "Kitty class maintenance"` and `hermes -p academy cron runs "Kitty class maintenance" --limit 5`. Empty successful stdout is silent; a non-zero script exit is retained as a failed run. This job uses no LLM and receives no Supabase or Meta credentials.
