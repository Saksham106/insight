...
Kitty reuses the existing approved `class_human_attention` Utility template. Its two variables are the recipient's messaging name and one service-generated, bounded class-update summary. The optional `WHATSAPP_TEMPLATE_CLASS_*` variables in `.env.example` remain dedicated overrides for a future rollout; they are not required and must not delay activation. The summary never contains raw inbound text or a free-form absence reason. A sender first selects and confirms the exact occurrence and scope. Only then does Insight reserve a notice. An individual absence, late arrival, or early departure updates only that enrollment and notifies the teacher; it does not cancel the group occurrence or identify the student to other families. A teacher-confirmed whole-class cancellation notifies every configured enrollment without waiting for family approval. A whole-class reschedule requires the teacher and one authorized decision for every active enrollment on the same request version. Final notices go to the teacher and each enrollment's configured recipients.

Run a shadow pilot while the flag remains false: use a disposable database or staging project and synthetic contacts to create one individual class and one group class, including a weekly series and a one-off. Snapshot the Academy tables before the pilot and prove they are unchanged afterward. Exercise student and parent absence reports, teacher cancellation, whole-class reschedule approval by every enrollment, ambiguous class and scope selection, stale versions, rejection, duplicate delivery, failed or indeterminate delivery, idempotent retry, privacy between families, Swati override, expiration/recovery, and the daily maintenance recovery job. Normal tool mutations drain up to 20 notices synchronously for immediate delivery; the daily cron reclaims interrupted work and handles quiet-period maintenance without requiring a paid high-frequency Vercel cron. Confirm that external contacts cannot create or edit classes and that the admin Classes tab shows Upcoming, Needs attention, Recurring, and History.

For hourly retry and quiet-period maintenance, use Hermes's built-in scheduler. Normal class updates still drain notifications immediately; this job is only a recovery path for interrupted delivery and housekeeping. Copy `scripts/kitty-class-maintenance.py` into the Academy profile's `$HERMES_HOME/scripts/` directory, set `INSIGHT_KITTY_MAINTENANCE_URL=https://<insight-host>/api/cron/kitty-classes` in that profile, and reuse its existing `HERMES_TOOL_SHARED_SECRET`. Create the deterministic no-agent job with `hermes -p academy cron create "every 1h" --no-agent --script kitty-class-maintenance.py --deliver local --name "Kitty class maintenance"`. Verify with `hermes -p academy cron run "Kitty class maintenance"` and `hermes -p academy cron runs "Kitty class maintenance" --limit 5`. Empty successful stdout is silent; a non-zero script exit is retained as a failed run. This job uses no LLM and receives no Supabase or Meta credentials.

Structured Academy routines use the same deterministic pattern because the Vercel Hobby plan supports only daily cron entries. Copy `scripts/agent-routine-maintenance.py` into the Academy profile's `$HERMES_HOME/scripts/` directory, set `INSIGHT_AGENT_ROUTINE_URL=https://<insight-host>/api/cron/agent-routines`, and reuse `HERMES_TOOL_SHARED_SECRET`. Create `hermes -p academy cron create "every 15m" --no-agent --script agent-routine-maintenance.py --deliver local --name "Academy agent routines"`. Verify it once with `hermes -p academy cron run "Academy agent routines"`, then inspect `hermes -p academy cron runs "Academy agent routines" --limit 5`. The endpoint re-evaluates current authority and relationships before every action and returns aggregate counts only; this scheduler uses no model tokens and receives no database or Meta credentials.

Before any live send, list environment-variable names without values and confirm the feature flag is false, the dedicated tool URL is exact, `WHATSAPP_TEMPLATE_HUMAN_ATTENTION` points to the already-approved template, and the existing Meta sender secrets remain configured. Use only a provider sandbox or explicitly selected contacts for the pilot. If a safe recipient or an exact environment value is missing, stop at the database/application pilot and leave the flag false; no new Meta template approval is required.
