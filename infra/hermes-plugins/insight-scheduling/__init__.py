    ...
    ctx.register_tool(
        ...
        schema={
            ...
            "description": (
                ...
                "If the sender cannot confirm the exact class or scope and Swati must resolve it, call "
                "report_class_ambiguity={candidateOccurrenceIds,ambiguityKind,clientRequestId} once using only the 1–5 "
                "occurrence IDs returned by find_my_classes and ambiguityKind class or scope. Never include message text, "
                "names, phone numbers, or a free-form reason in that payload. Do not call it while clarification is still possible. "
                ...
                "Create one clientRequestId per intended mutation. Reuse that exact clientRequestId after a timeout or uncertain response; never derive it from a transport request ID. "
